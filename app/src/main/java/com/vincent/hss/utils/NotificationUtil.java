...
import com.vincent.hss.activity.ChatActivity;
...
//				.setNumber(number)//显示数量
     ...
     * @param c
     */
    public  static final void sendNotificationAskMsg(Context context,Class c,String fromPhone,String content){
        NotificationManager mNotificationManager= (NotificationManager) context.getSystemService(NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context);
        mBuilder.setContentTitle("来自"+fromPhone+"的添加请求")
...
//				.setNumber(number)//显示数量
        ...
        //点击的意图ACTION是跳转到Intent
        Intent resultIntent = new Intent(context, c);
        resultIntent.putExtra("fromPhone",fromPhone);
        resultIntent.putExtra("content",content);
        resultIntent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0,resultIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        mBuilder.setContentIntent(pendingIntent);
    ...
    public  static final void sendNotificationChatMsg(Context context,String ask_phone,String chatJson){
        ...
        mBuilder.setContentTitle(ask_phone+"给你发消息啦，点击查看")
                .setContentText("有人找你啦")
                .setContentIntent(getDefalutIntent(context,Notification.FLAG_AUTO_CANCEL))
//				.setNumber(number)//显示数量
                .setTicker("有人找你啦")//通知首次出现在通知栏，带上升动画效果的
                .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示
                .setPriority(Notification.PRIORITY_DEFAULT)//设置该通知优先级
                .setAutoCancel(true)//设置这个标志当用户单击面板就可以让通知将自动取消
                .setOngoing(false)//ture，设置他为一个正在进行的通知。他们通常是用来表示一个后台任务,用户积极参与(如播放音乐)或以某种方式正在等待,因此占用设备(如一个文件下载,同步操作,主动网络连接)
        ...
        resultIntent.putExtra("ask_phone",ask_phone);
        resultIntent.putExtra("chatJson",chatJson);
        resultIntent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0,resultIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        mBuilder.setContentIntent(pendingIntent);
        mNotificationManager.notify((int) System.currentTimeMillis(), mBuilder.build());
